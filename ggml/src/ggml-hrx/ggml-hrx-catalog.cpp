...
static bool ggml_backend_hrx_parse_one_prepass(
        const nlohmann::json & prepass,
        const std::string & route_id,
        ggml_backend_hrx_catalog_prepass * out,
        std::string * out_error) {
    out->artifact_id = prepass.value("artifact_id", std::string());
    out->root_symbol = prepass.value("root_symbol", std::string());
    out->export_name = prepass.value("export_name", std::string());
    out->src_index = static_cast<uint32_t>(prepass.value("src_index", 1));
    const auto src_indices = prepass.find("src_indices");
    if (src_indices != prepass.end()) {
        if (prepass.contains("src_index") ||
            !src_indices->is_array() || src_indices->empty()) {
            if (out_error) {
                *out_error = "route " + route_id +
                    " prepass src_indices must be a nonempty array and cannot accompany src_index";
            }
            return false;
        }
        for (const auto & source : *src_indices) {
            if (!source.is_number_integer()) {
                if (out_error) {
                    *out_error = "route " + route_id +
                        " prepass src_indices must contain integers";
                }
                return false;
            }
            const int64_t index = source.get<int64_t>();
            if (index < 0 || index > 0xffffffffll ||
                std::find(
                    out->src_indices.begin(),
                    out->src_indices.end(),
                    static_cast<uint32_t>(index)) !=
                    out->src_indices.end()) {
                if (out_error) {
                    *out_error = "route " + route_id +
                        " prepass src_indices contains an invalid or duplicate index";
                }
                return false;
            }
            out->src_indices.push_back(static_cast<uint32_t>(index));
        }
        out->src_index = out->src_indices.front();
    } else {
        out->src_indices.push_back(out->src_index);
    }
    out->dst_index = static_cast<int32_t>(prepass.value("dst_index", -1));
    out->bytes_per_element = static_cast<uint32_t>(prepass.value("bytes_per_element", 0));
    out->element_binding_key = prepass.value("element_binding_key", std::string());
    out->persistent = prepass.value("persistent", false);
    out->scratch_class = prepass.value("scratch_class", std::string());
    if (out->artifact_id.empty() || out->root_symbol.empty()) {
        if (out_error) {
            *out_error = "route " + route_id + " prepass needs artifact_id and root_symbol";
        }
        return false;
    }
    const auto scratch = prepass.find("scratch_elements");
    if (scratch == prepass.end() || !scratch->is_array() || scratch->empty()) {
        if (out_error) {
            *out_error = "route " + route_id + " prepass needs a scratch_elements array";
        }
        return false;
    }
    for (const auto & source : *scratch) {
        if (!source.is_string()) {
            if (out_error) {
                *out_error = "route " + route_id + " prepass scratch_elements must be strings";
            }
            return false;
        }
        out->scratch_element_sources.push_back(source.get<std::string>());
    }
    if (out->dst_index >= 0 &&
        (out->bytes_per_element == 0 ||
         out->persistent ||
         !out->scratch_class.empty())) {
        if (out_error) {
            *out_error = "route " + route_id +
                " direct-output prepass needs one nonpersistent output without scratch_class";
        }
        return false;
    }
    const auto bindings = prepass.find("bindings");
    if (bindings != prepass.end()) {
        if (!bindings->is_array()) {
            if (out_error) {
                *out_error = "route " + route_id + " prepass bindings must be an array";
            }
            return false;
        }
        for (const auto & binding : *bindings) {
            if (!binding.is_object() || !binding.contains("key") || !binding["key"].is_string()) {
                if (out_error) {
                    *out_error = "route " + route_id + " prepass binding needs a string key";
                }
                return false;
            }
            ...
            if (binding.contains("source") && binding["source"].is_string()) {
                ...
                parsed.shape_source = parsed.source.compare(0, 6, "shape.") == 0
                    ? parsed.source.substr(6) : parsed.source;
            }
            if (binding.contains("value")) {
                parsed.value = binding["value"].is_string()
                    ? binding["value"].get<std::string>()
                    : binding["value"].dump();
            }
            out->bindings.push_back(std::move(parsed));
        }
    }
    out->enabled = true;
    return true;
...
static bool ggml_backend_hrx_parse_prepass(
        const nlohmann::json & route,
        ggml_backend_hrx_catalog_route * out_route,
        std::string * out_error) {
    const auto single = route.find("prepass");
    if (single != route.end()) {
        if (!single->is_object()) {
            if (out_error) {
                *out_error = "route " + out_route->id + " prepass must be an object";
            }
            return false;
        }
        ggml_backend_hrx_catalog_prepass parsed;
        if (!ggml_backend_hrx_parse_one_prepass(*single, out_route->id, &parsed, out_error)) {
            return false;
        }
        out_route->prepasses.push_back(std::move(parsed));
    }
    const auto many = route.find("prepasses");
    if (many != route.end()) {
        if (!many->is_array()) {
            if (out_error) {
                *out_error = "route " + out_route->id + " prepasses must be an array";
            }
            return false;
        }
        for (const auto & entry : *many) {
            if (!entry.is_object()) {
                if (out_error) {
                    *out_error = "route " + out_route->id + " prepasses entries must be objects";
                }
                return false;
            }
            ggml_backend_hrx_catalog_prepass parsed;
            if (!ggml_backend_hrx_parse_one_prepass(entry, out_route->id, &parsed, out_error)) {
                return false;
            }
            out_route->prepasses.push_back(std::move(parsed));
        }
...
static bool ggml_backend_hrx_parse_catalog_json(
    ...
    for (const auto & route_json : value["routes"]) {
        ...
        if (!ggml_backend_hrx_parse_prepass(route_json, &route, out_error)) {
            return false;
        }
...
const ggml_backend_hrx_catalog_route * ggml_backend_hrx_catalog_find_route(
    ...
    for (const size_t route_index : index_it->second) {
        ...
        // The dynamic matmul kernels derive their grid from push constants, so
        // their launch config cannot be resolved at compile time and this backend
        // cannot launch them. Skipping them here keeps supports_op in step with
        // dispatch: without it the op is claimed, dispatch fails, and the
        // destination is left holding whatever was in the buffer.
        if (route.constant_byte_length != 0 &&
            (problem.op == "MUL_MAT" || problem.op == "MUL_MAT_ID")) {
            continue;
        }
        bool match = true;
...
bool ggml_backend_hrx_catalog_make_config_bindings(
    ...
    for (const auto & binding : route.bindings) {
        ...
        if (!binding.value.empty()) {
            ...
            const auto shape_it = problem.shape.find(binding.shape_source);

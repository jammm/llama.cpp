...
def require_unique_target_route(entries: list[dict[str, Any]]) -> None:
    seen: set[tuple[str, str]] = set()
    for route in entries:
        target_key = route.get("target_key")
        route_id = route.get("id")
        require(isinstance(target_key, str) and target_key, "route entry missing target_key")
        require(isinstance(route_id, str) and route_id, "route entry missing id")
        key = (target_key, route_id)
        require(key not in seen, f"duplicate route for target {target_key}: {route_id}")
        seen.add(key)
...
def load_split_routes(catalog_dir: Path) -> list[dict[str, Any]]:
    ...
    for path in sorted(target_dir.rglob("*.json")):
        ...
        for index, route in enumerate(file_routes):
            ...
            routes.append(row)
    require_unique_target_route(routes)
    return routes
...
def assemble(catalog_dir: Path) -> OrderedDict[str, Any]:
    ...
    for schedule in catalog.get("test_schedules", []):
        ...
        target_key = schedule.get("target_key")
        for case in schedule.get("cases", []):
            expected_route_id = case.get("expected_route_id")
            require(
                (target_key, expected_route_id) in route_ids,

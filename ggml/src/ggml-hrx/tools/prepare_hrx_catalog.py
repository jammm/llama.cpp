...
def artifact_roots(catalog: dict[str, Any]) -> dict[str, list[str]]:
    ...
    for route in catalog.get("routes") or []:
        ...
        prepasses: list[Any] = []
        if route.get("prepass") is not None:
            prepasses.append(route["prepass"])
        value = route.get("prepasses")
        if isinstance(value, list):
            prepasses.extend(value)
        for prepass in prepasses:
            if not isinstance(prepass, dict):
                continue
            artifact_id = prepass.get("artifact_id")
            root_symbol = prepass.get("root_symbol")
            if artifact_id and root_symbol:
                root_list = roots.setdefault(str(artifact_id), [])
                if str(root_symbol) not in root_list:
                    root_list.append(str(root_symbol))
...
def link_artifacts(processed_dir: Path, catalog: dict[str, Any], loom_link: Path) -> None:
    ...
    link_groups: dict[Path, dict[str, Any]] = {}
    for artifact_id, artifact in artifacts.items():
        ...
        link_config = artifact.get("link_config", {})
        require(
            isinstance(link_config, dict),
            f"artifact {artifact_id} link_config must be an object",
        )
        for key, value in link_config.items():
            require(
                isinstance(key, str) and key and not key.startswith("@"),
                f"artifact {artifact_id} link_config keys must be non-empty Loom names without @",
            )
            require(
                isinstance(value, str) and value,
                f"artifact {artifact_id} link_config values must be non-empty strings",
            )
        ...
        require(
            group["source_id"] == source_id and group["source_path"] == source_path,
            f"artifact path {artifact_path} is shared by different sources",
        )
        require(
            list(group["link_config"].items()) == list(link_config.items()),
            f"artifact path {artifact_path} is shared by different link_config values",
        )
        group["artifact_ids"].append(str(artifact_id))
        for root in roots:
            if root not in group["roots"]:
                group["roots"].append(root)

    for artifact_path, group in link_groups.items():
        ...
        cmd.extend(f"--root={root}" for root in group["roots"])
        cmd.extend(
            f"--config={key}={value}"
            for key, value in group["link_config"].items()
        )
        ...
        if result.returncode != 0:
            ...
            if fallback.returncode != 0:
                ...
                raise SystemExit(
                    "failed to link HRX catalog artifacts "
                    + ", ".join(group["artifact_ids"])
                )
            sys.stderr.write(
                "warning: linked HRX catalog artifacts "
                + ", ".join(group["artifact_ids"])
                + " without --strip-check "
